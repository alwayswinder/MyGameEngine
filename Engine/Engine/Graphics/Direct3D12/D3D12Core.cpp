...
namespace primal::graphics::d3d12::core
{
	namespace
	{
		...
		public:
			...
			explicit d3d12_command(ID3D12Device6 *const device, D3D12_COMMAND_LIST_TYPE type)
			{
				HRESULT hr{ S_OK };
			...
			~d3d12_command()
			{
				assert(!_cmd_list && !_cmd_queue && !_fence);
			}
			...
			constexpr ID3D12GraphicsCommandList5* const command_list()const { return _cmd_list; }
		...
		ID3D12Device6*				main_device{ nullptr };
		...
		IDXGIAdapter4* determine_main_adapter()
		{
			...
			for (u32 i{ 0 };
				...
				++i)
			{
				if (SUCCEEDED(D3D12CreateDevice(adapter, mininum_feature_level, __uuidof(ID3D12Device), nullptr)))
				{
					return adapter;
				}
				release(adapter);
		...
		D3D_FEATURE_LEVEL get_max_feature_level(IDXGIAdapter4* adapter)
		{
			...
			DXCall(D3D12CreateDevice(adapter, mininum_feature_level, IID_PPV_ARGS(&device)));
	...
	bool initialize()
	{
		...
		assert(max_feature_level >= mininum_feature_level);
		if (max_feature_level < mininum_feature_level) return failed_init();
	...
	void render()
	{
		...
		ID3D12GraphicsCommandList5* cmd_list{ gfx_command.command_list() };
