...
namespace primal::graphics::d3d12
{
	...
	bool descriptor_heap::initialize(u32 capacity, bool is_shader_visiable)
	{
		...
		desc.Flags = is_shader_visiable ? D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE : D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
		...
		_gpu_start = is_shader_visiable ? _heap->GetGPUDescriptorHandleForHeapStart() : D3D12_GPU_DESCRIPTOR_HANDLE{ 0 };
		return true;
	}
	...
	{
		assert(info.desc);
		_mip_cout = resource()->GetDesc().MipLevels;
		assert(_mip_cout && _mip_cout <= d3d12_texture::max_mips);
	...
	{
		for (u32 i{ 0 }; i < _mip_cout; ++i) core::rtv_heap().free(_rtv[i]);
		_texture.release();
		_mip_cout = 0;
	}
	/////////////////////////////////////////////Depth_Buffer
